...
type Store struct {
	...
	StoreCode			string				`gorm:"type:varchar(100);not null;uniqueIndex:idx_store_code"`
	StoreName			string				`gorm:"type:varchar(100);not null;uniqueIndex:idx_store_name_deleted_at,priority:1"`
	BranchName			string				`gorm:"type:varchar(100);not null;index"`
	Description			*string				`gorm:"type:text"`
	PrimaryPhone		string				`gorm:"type:varchar(50);not null;uniqueIndex:idx_primary_phone_deleted_at,priority:1"`
	...
	DeletedAt 			gorm.DeletedAt		`gorm:"uniqueIndex:idx_primary_phone_deleted_at,priority:2;uniqueIndex:idx_store_name_deleted_at,priority:2"`	
